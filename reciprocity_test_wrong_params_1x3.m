%% reciprocity_test_wrong_params_1x3.m
% Reciprocity check with deliberately mismatched current-drive parameters.
%
% For each model:
% 1) Apply sinusoidal voltage v(t), measure current i(t).
% 2) Re-apply i(t) to a perturbed version of the model.
% 3) Reconstruct output voltage v_out(t).
% 4) Save one 1x3 figure and one separate error figure.
...
% Output folder
% ============================================================
outdir = fullfile('results','figures','reciprocity_test_wrong_params');
...
mismatchNotes = cell(numel(models),1);
...
for m = 1:numel(models)
    ...
    [currentModel.params, mismatchNotes{m}] = make_wrong_current_drive_params(models(m));
    out_i = simulate_current_driven_model(t,out_v.I,currentModel);
    ...
    sgtitle([models(m).name ' Mismatched Reciprocity Test'], ...
    ...
    title([models(m).name ' Mismatched Reciprocity Error'], 'FontWeight','normal');
    grid on; box off;
...
summaryTable = table({models.name}', mismatchNotes, rmse, maxAbsError, ...
    'VariableNames', {'Model','CurrentDriveMismatch','RMSE_V','MaxAbsError_V'});
writetable(summaryTable, fullfile(outdir,'reciprocity_error_summary.csv'));

disp(summaryTable);
fprintf('\nMismatched reciprocity figures saved in:\n%s\n', outdir);

%% ============================================================
% Local function: deliberately perturb current-drive parameters
% ============================================================
function [params, note] = make_wrong_current_drive_params(model)

    params = model.params;

    switch model.type
        case 'hp_charge'
            params.R0 = 1.35*params.R0;
            params.k_eff = 0.55*params.k_eff;
            note = 'R0 x1.35, k_eff x0.55';

        case 'hp_state'
            params.x0 = 0.35;
            params.mu = 1.80*params.mu;
            note = 'x0 -> 0.35, mu x1.80';

        case 'joglekar'
            params.x0 = 0.65;
            params.mu = 1.40*params.mu;
            params.p = 2;
            note = 'x0 -> 0.65, mu x1.40, p -> 2';
        ...
        case 'prodromakis'
            ...
            params.p1 = 0.55;
            params.p2 = 4;
            note = 'x0 -> 0.45, mu x1.40, p1 -> 0.55, p2 -> 4';

        case 'chalcogenide'
            if strcmp(model.slug,'Chalcogenide_MSS_nonlinear')
                params.X0 = 0.55;
                params.tau = 0.8e-3;
                params.beta = 40;
                params.V_on = 0.20;
                params.V_off = 0.34;
                note = 'X0 -> 0.55, tau -> 0.8e-3, beta -> 40, thresholds shifted';
            else
                params.X0 = 0.45;
                params.tau = 1.5e-3;
                params.beta = 20;
                params.V_on = 0.30;
                params.V_off = 0.18;
                note = 'X0 -> 0.45, tau -> 1.5e-3, beta -> 20, thresholds shifted';
            end
