%% HP_v2_final_fixed.m
% HP v2 model: charge-controlled HP model
%
% Produces:
% 1) I-V response at 50 Hz
% 2) I-V response at different frequencies
% 3) q-phi response at 50 Hz
% 4) I-V response at different amplitudes
...
% Output folder
% ============================================================
outdir = 'results/figures/HP_v2_final_fixed';
...
% Match your good HP v3 initial memristance
R0 = params.Roff/10;       % 3000 ohm

% Positive coefficient gives upward q-phi curve:
% M(q) = R0 + k*q
k_eff = (params.Roff - params.Ron) * params.mu * params.Ron / params.D^2;
...
out_main = HPmodel_v2_charge_controlled(t,V_main,R0,k_eff);
...
title('HP v2 model: I-V response at 50 Hz');
grid on;
box off;

format_and_save_figure(hfig, fullfile(outdir,'HPv2_IV_50Hz'));

%% ============================================================
% Figure 2: I-V response at different frequencies
% ============================================================
...
for kk = 1:length(freqs)
    ...
    out = HPmodel_v2_charge_controlled(t,V,R0,k_eff);
    I = out.I;
...
xlabel('Voltage, v(t) [V]');
ylabel('Current, i(t) [A]');
title('HP v2 model: I-V response at different frequencies');
legend('Location','best');
grid on;
box off;

format_and_save_figure(hfig, fullfile(outdir,'HPv2_IV_frequency_sweep'));

%% ============================================================
% Figure 3: q-phi response at 50 Hz
% ============================================================
hfig = figure;
...
title('HP v2 model: q-\phi response at 50 Hz');
grid on;
box off;

format_and_save_figure(hfig, fullfile(outdir,'HPv2_qphi_50Hz'));

%% ============================================================
% Figure 4: I-V response at different amplitudes
% ============================================================
...
xlabel('Voltage, v(t) [V]');
ylabel('Current, i(t) [A]');
title('HP v2 model: I-V response at different amplitudes');
legend('Location','best');
grid on;
box off;

format_and_save_figure(hfig, fullfile(outdir,'HPv2_IV_amplitude_sweep'));

%% ============================================================
% Figure 5: memristance response at 50 Hz
% ============================================================
hfig = figure;
...
title('HP v2 model: memristance response at 50 Hz');
grid on;
box off;

format_and_save_figure(hfig, fullfile(outdir,'HPv2_memristance_50Hz'));
...
xlabel('Time, t [s]');
title('HP v2 model: input voltage and output current at 50 Hz');
grid on;
box off;

format_and_save_figure(hfig, fullfile(outdir,'HPv2_voltage_current_time_50Hz'));
...
title('HP v2 model: hysteresis area versus frequency');
grid on;
box off;

format_and_save_figure(hfig, fullfile(outdir,'HPv2_area_vs_frequency'));
...
xlabel('Amplitude [V]');
ylabel('I-V loop area, |\oint i dv| [VA]');
title('HP v2 model: hysteresis area versus amplitude');
grid on;
box off;

format_and_save_figure(hfig, fullfile(outdir,'HPv2_area_vs_amplitude'));
...
writetable(frequencyTable, fullfile(outdir,'HPv2_frequency_sweep_summary.csv'));
writetable(amplitudeTable, fullfile(outdir,'HPv2_amplitude_sweep_summary.csv'));
...
%% ============================================================
% Local function: HP v2 charge-controlled model
% ============================================================
function out = HPmodel_v2_charge_controlled(t,V,R0,k_eff)
    ...
    for n = 1:N-1
        ...
        % Safety check
        if M(n) <= 0
            warning('HP v2: memristance became non-positive. Check parameters.');
            M(n) = NaN;
        end
    ...
    if M(N) <= 0
        warning('HP v2: final memristance became non-positive. Check parameters.');
        M(N) = NaN;
    end
