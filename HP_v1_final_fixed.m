%% HP_v1_final_fixed.m
% HP v1 model: analytical voltage-history HP model
%
% Produces:
...
% Output folder
% ============================================================
outdir = 'results/figures/HP_v1_final_fixed';
...
out_main = HPmodel_v1_analytical(t,V_main,R0,k_eff);
...
for kk = 1:length(freqs)
    ...
    out = HPmodel_v1_analytical(t,V,R0,k_eff);
    I = out.I;
...
%% ============================================================
% Local function: HP v1 analytical voltage-history model
% ============================================================
function out = HPmodel_v1_analytical(t,V,R0,k_eff)

    % Flux from voltage
    phi = cumtrapz(t,V);

    % Positive sign selected so that q-phi curves upward:
    % M^2 = R0^2 + 2*k_eff*phi
    sqrt_arg = R0^2 + 2*k_eff*phi;

    if any(sqrt_arg <= 0)
        warning('HP v1: square-root argument became non-positive. Check parameters.');
